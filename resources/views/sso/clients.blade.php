        ...
        $(document).ready(function() {
                        ...
                        render: function(data, type, row) {
                            return `
                                <div class="secret-container">
                                    <span class="secret-text" style="display: none;">${data}</span>
                                    <span class="secret-placeholder">••••••••</span>
                                    <button class="toggle-secret bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded ml-2">
                                        <i class="fas fa-eye"></i> Show
                                    </button>
                                    <button class="copy-secret bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-2 rounded ml-2" style="display: none;">
            ...
            $('#saveClient').click(function(e) {
                ...
                $.ajax({
                    ...
                    <p>
                        Client Secret: 
                        <input type="password" id="clientSecret" value="${response.client_secret}" readonly style="width: 60%; border: none; background: transparent;">
                        <button id="toggleSecret" class="ml-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded">
                            <i class="fas fa-eye"></i> Show
                        </button>
                        <button id="copySecret" class="ml-2 bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-2 rounded" style="display: none;">
                            <i class="fas fa-copy"></i> Copy
                        </button>
                    </p>
                ...
                `,
                            icon: 'success',
                            confirmButtonText: 'I have saved the secret',
                            didRender: () => {
            ...
            $(document).on('click', '.toggle-secret', function() {
                ...
                var secretText = container.find('.secret-text');
                var placeholder = container.find('.secret-placeholder');
                var copyButton = container.find('.copy-secret');

                if (secretText.is(':visible')) {
                    secretText.hide();
                    placeholder.show();
            ...
            $(document).on('click', '.copy-secret', function() {
                var secretText = $(this).closest('.secret-container').find('.secret-text').text();
                navigator.clipboard.writeText(secretText).then(function() {
                    Swal.fire({
                        title: 'Copied!',
                        text: 'Client secret has been copied to clipboard.',
                        icon: 'success',
                        timer: 1500,
